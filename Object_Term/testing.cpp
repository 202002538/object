...
bool paircheck(string exp) //이게지금 옳은 수식인가? 괄호를 제대로 쳤냐고.
{
    ...
    for (int i = 0; i < exp.size(); i++)
    {
        ...
        if (i < exp.size() - 1 && !isdigit(symbol)) //연속으로 연산자x. 맨 마지막글자가 연산자인 경우 마지막+1번째를 비교하지 않록 장치
            if (!isdigit(exp[i + 1]))
                return false;
        ...
        {
        case '(':case '[':case '{':
            if (i < exp.size() - 1 && exp[i + 1] == '-') //괄호 바로다음 -가 나오면 음수
                return false;
            push(symbol);
            break;
        case ')':case ']':case '}':
...
string postfix(string exp) //후위표기식으로 변환 
{
    ...
    for (int i = 0; i < exp.size(); i++)
    {
        ...
        {
            if(isdigit(exp[i+1])) //다음에 온것도 숫자면?
                //두자리수 이상인거라는 거지. 연산자 나올때까지의 숫자들을 묶어서 어케해야한다는 거지.
            result += symbol;
            continue;
        }
...
int eval(string exp)
{
    ...
    for (int i = 0; i < exp.size(); i++)
    {
        ...
        {
            push((element)(symbol - '0')); //0을 빼는 것은 char을 int로 바꾸기 위함
        }
        else
        {
            element opr2 = pop();
...
void test(string exp) //들어온 수식이 옳은 식인지 판단 후, 후위표기식으로 바꿈->계산
{
    ...
    while (!isEmpty()) pop();
    if (!paircheck(exp)) //괄호의 열고닫기, 음수여부를 검사한다. 
    {
        cout << "잘못된 수식 " << endl;
        return;
    }
